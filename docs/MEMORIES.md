...
- **2026-09-19**:
  - *Spring Security Mocking Pitfall*: Do NOT use `@MockitoBean` directly on a custom security filter (e.g., `JwtAuthenticationFilter`) inside a `@WebMvcTest`. By default, Mockito intercepts the filter but does NOT invoke `filterChain.doFilter()`. This causes the request to silently stop at the filter and return a false-positive `HTTP 200 OK` with an empty JSON body, leading to massive debugging confusion. **Solution**: Use `@Import({SecurityConfig.class, JwtAuthenticationFilter.class})` to load the *real* filter bean, and only mock its underlying dependencies (e.g., `JwtService`).
